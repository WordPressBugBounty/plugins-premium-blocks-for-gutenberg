...
window.addEventListener("DOMContentLoaded", () => {
  ...
  Object.keys(forms).forEach(formId => {
    ...
    Object.keys(innerBlocks).forEach(blockId => {
      ...
      const innerBlock = document.querySelector(`.${blockId}`);
      if (attrs.iconTypeSelect === "svg") {
        const svg = innerBlock.querySelector(`.premium-form-input-svg-class`);
  ...
  const submitForm = (formId, attributes, innerBlocks, formElement, reCaptchaResponse = "") => {
    ...
    if (submitActions?.mailchimp) {
      let mappedFields = [];
      if (attributes.mailchimpMappedFields.length) {
        mappedFields = attributes.mailchimpMappedFields;
      } else {
        /**
         * Backward compatibility with old version that use mailchimpFirstNameField and mailchimpLastNameField attributes
         * manually generating mappedFields corresponding to the old used merge tags.
         */
        mappedFields = [{
          field_name: "",
          field_tag: "FNAME"
      ...
      /**
       * Backward compatibility with old version that use mailchimpFirstNameField and mailchimpLastNameField attributes
       * It maps the fields with the corresponding attributes and returns the updated mapped fields.
       */
      const updatedMappedFields = mappedFields.map(field => {
        if (field.field_tag === "FNAME" && attributes.mailchimpFirstNameField && field.field_name === "") {
          return {
            ...field,
            field_name: attributes.mailchimpFirstNameField
          };
        }
        if (field.field_tag === "LNAME" && attributes.mailchimpLastNameField && field.field_name === "") {
          return {
            ...field,
            field_name: attributes.mailchimpLastNameField
          };
        }
        return field;
      });
      ...
      updatedMappedFields.forEach(({
        field_name
      }) => {
        const fieldElement = formElement.querySelector(`input[name="${field_name}"]`);
        if (field_name.startsWith("phone")) {
          const countryCodeField = formElement.querySelector("select.premium-form-phone-code-select");
          if (fieldElement && countryCodeField) {
            mailchimpSettings[field_name] = `(${countryCodeField.value}) ${fieldElement.value}`;
          }
          return;
        }
        if (fieldElement) {
          mailchimpSettings[field_name] = fieldElement.value;
        }
      });

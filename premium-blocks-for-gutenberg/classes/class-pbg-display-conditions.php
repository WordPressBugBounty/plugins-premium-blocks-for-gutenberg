...
class PBG_Display_Conditions
{
    ...
    private function __construct()
    {
        ...
        if (! is_admin()) {
            ...
            add_action('wp_footer', array($this, 'set_returning_visitor_cookie'));
        }
    ...
    public function pbg_render_block($block_content, $block)
    {
        ...
        if (empty($display_conditions) || empty($display_conditions['conditions'] ?? array()) || ! $display_conditions['enabled'] || (defined('REST_REQUEST') && REST_REQUEST && isset($_REQUEST['context']) && 'edit' === sanitize_key(wp_unslash($_REQUEST['context'])))) {
    ...
    public static function get_user_ip_address()
    {
        ...
        $trusted_headers = (array) apply_filters('pbg_trusted_proxy_headers', array());

        $allowed_header_names = array(
            'X-Forwarded-For',
            'CF-Connecting-IP',
            'X-Real-IP',
            'True-Client-IP',
        ...
        foreach ($trusted_headers as $header) {
            if (! in_array($header, $allowed_header_names, true)) {
                continue;
            }
            ...
            // Only accept public, non-reserved IPs from forwarded headers.
    ...
    public static function get_timezone_by_ip($ip_address)
    {
        ...
        if (empty($location_data['location']['time_zone'])) {
            return date_default_timezone_get();
        }
    ...
    public function compare_location_value($operator, $value, $method)
    {
        ...
        if ('0.0.0.0' === $ip_address || empty($ip_address)) {
            return false;
        }
        if ('old' === $method) {

            $raw_data      = rplg_urlopen('http://www.geoplugin.net/php.gp?ip=' . $ip_address)['data'] ?? '';
            $location_data = (is_string($raw_data) && false === strpos($raw_data, 'O:')) ? unserialize($raw_data) : false; // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.serialize_unserialize

            if (! is_array($location_data) || 404 === $location_data['geoplugin_status']) {
                return false; // localhost or bad response.
            }
            ...
            if (empty($location_data['country']['names']['en'])) {
                return false; // localhost or incomplete API response.
            }
    ...
    /**
     * Set a cookie on the frontend so returning visitor condition works on subsequent visits.
     */
    public function set_returning_visitor_cookie()
    {
        $page_id = get_the_ID();
        ...
        $cookie_name = 'isReturningVisitor' . absint($page_id);
        ?>
		<script>
			(function() {
				var name = '<?php echo esc_js($cookie_name); ?>';
				if (!document.cookie.split(';').some(function(c) {
						return c.trim().startsWith(name + '=');
					})) {
					var expires = new Date();
					expires.setFullYear(expires.getFullYear() + 1);
					document.cookie = name + '=1; path=/; expires=' + expires.toUTCString() + '; SameSite=Lax';
				}
			})();
		</script>
<?php
